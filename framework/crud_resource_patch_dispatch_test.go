...
func Test_CRUDResource_PatchDispatch(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		if err != nil {
			t.Fatal("test %d: unexpected NewCRUDResource error = %#v", i, err)
		}

		err = r.EnsureCreated(context.Background(), nil)
		if err != nil {
			t.Fatal("test %d: unexpected EnsureCreated error = %#v", i, err)
		}

		err = r.EnsureDeleted(context.Background(), nil)
		if err != nil {
			t.Fatal("test %d: unexpected EnsureDeleted error = %#v", i, err)
		}
	}
}
