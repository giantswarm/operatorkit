...
func Test_Finalizer_Integration_Basic(t *testing.T) {
	namespace := "finalizer-integration-basic-test"
	mustSetup(namespace)
	...
	operatorkitFramework, err := newFramework(operatorName, namespace, nil)
	if err != nil {
	...
	createdConfigMap, err := createConfigMap(namespace, cm)
	if err != nil {
		t.Fatal("expected", nil, "got", err)
	}
	...
	resultConfigMap, err := getConfigMap(namespace, configMapName)
	if err != nil {
		t.Fatal("expected", nil, "got", err)
	}
	...
	err = deleteConfigMap(namespace, configMapName)
	if err != nil {
		t.Fatal("expected", nil, "got", err)
	}
	resultConfigMap, err = getConfigMap(namespace, configMapName)
	...
	_, err = getConfigMap(namespace, configMapName)
